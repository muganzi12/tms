...
class DecliningBalanceAmortization extends Model {
    ...
    public function getTheCurrentInterval($interval){
        ...
        if($interval%4 == 0){
            ...
            $finalInterval = floor($interval/4) + 1;
        }
    ...
    public function getBreakdownByMonth() {
        ...
        $payments[0] = [
            ...
            'next_date' => date("Y-m-d", $this->start_date)
        ];
        ...
        ///CONVERT MONTHS TO WEEKS EACH OF 4WEEKS
            ...
            $payments[1] = [
               ...
               'interest' => $payments[0]['opening_balance'] * $this->interest_rate,
                'opening_balance' => $payments[0]['opening_balance'] - ($this->monthlyInstallmentPayment-$payments[$i-1]['opening_balance']*$this->interest_rate),
        ...
        for ($i = 4; $i <= $weeks; $i++) {
            $payments[$i+1] = [
                ...
                'interest' => ($payments[$this->getTheCurrentInterval($i+1)]['opening_balance'] * $this->interest_rate)/4,
                'opening_balance' =>($payments[$this->getTheCurrentInterval($i+1)]['opening_balance']) - ($this->monthlyInstallmentPayment-$payments[$i-1]['opening_balance']*$this->interest_rate),
                'principal'=>($this->monthlyInstallmentPayment-$payments[$i-1]['opening_balance']*$this->interest_rate)/4,
                'interest_rate' => $this->interest_rate,
            ];
